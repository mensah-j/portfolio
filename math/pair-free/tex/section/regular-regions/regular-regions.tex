...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    If not, then the square contains $(0, -1)$. 
    Since the boundary is a cycle, this cannot be another boundary point, so it must be an interior point, which implies $(-1, 0) \in R$.
    For the same reason, this is also an interior point.
    
    It follows that $e_{i+1} = \{q_{i+1}, q_{i+2}\} \subseteq B'(-1, 0)$ is good.
  \end{enumerate}
...
\begin{proof}
  ...
  Note that $C$ must have an adjacent point $q \in R \setminus C$.
  If this were not the case, then $C$ would be a connected component of $R$, implying $R = C$, which is impossible since $R$ must have nonempty interior. 
