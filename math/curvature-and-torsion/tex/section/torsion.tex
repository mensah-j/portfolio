...
\end{theorem}
\begin{proof}
  Given $s \in [0, 1]$ or $t \in [0, t(\gamma)]$, we view the maps $H(s, -)$ and $H(-, t)$ as functors between path categories; when applied to straight-line paths in their respective domains, these yield \say{vertical} or \say{horizontal} paths in $M$. Let $n > 0$ be an integer. By Proposition \ref{}, the fact that the homotopy fixes endpoints, and Proposition \ref{}, the torsion accumulated over $\gamma = H_1$ is equal to 
  \begin{align}
...
\input{section/torsion-approximation}

\input{section/euclidean-lemmas}