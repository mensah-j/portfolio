...
const TEX_DIR = (0, path_1.join)(process.cwd(), "lib/tikzjax/tex");
...
async function load() {
  if (!coredump) {
    const stream = (0, fs_1.createReadStream)(COREDUMP_PATH).pipe(
      (0, zlib_1.createGunzip)(),
    );
    coredump = await stream2buffer(stream);
  }
  if (!bytecode) {
    const stream = (0, fs_1.createReadStream)(BYTECODE_PATH).pipe(
      (0, zlib_1.createGunzip)(),
    );
...
async function extractTexFilesToMemory() {
  ...
  fs.mkdirSync("/lib");
  const stream = (0, fs_1.createReadStream)(TEX_FILES_PATH)
    .pipe((0, zlib_1.createGunzip)())
    .pipe(
      (0, tar_fs_1.extract)(TEX_FILES_EXTRACTED_PATH, {
        fs,
      }),
    );
